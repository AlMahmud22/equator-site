...
export default function SettingsPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (profileResponse.ok) {
          ...
          if (profileData.success && profileData.data) {
            ...
            setSettings({
              ...
              language: 'en',
              profileVisibility: profile.preferences?.privacy?.showEmail ? 'public' : 'private',
        ...
        if (apiKeysResponse.ok) {
          ...
          setApiKeys(apiKeysData.data?.apiKeys || [])
        }
  ...
  const handleCreateApiKey = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          setGeneratedApiKey(result.data.apiKey) // The actual key (only shown once)
          setApiKeys(prev => [...prev, {
            _id: result.data.keyId,
  ...
  const handleTerminateSession = async (sessionToken: string) => {
    try {
      ...
      if (response.ok) {
        setActiveSessions(prev => prev.filter(session => !session.sessionToken?.startsWith(sessionToken.split('...')[0])))
                ...
                <div className="bg-secondary-900/50 backdrop-blur-sm border border-secondary-700/50 rounded-xl p-6">
                  ...
                  {activeTab === 'api-keys' && (
                    <div className="space-y-6">
                      ...
                      <p className="text-secondary-300">
                        API keys allow your desktop applications to authenticate with equator services.
                      </p>
                      ...
                      {activeSessions.length > 0 ? (
                        <div className="space-y-4">
                          {activeSessions.map((session, index) => (
                            <div key={session.sessionToken} className="bg-secondary-800/30 rounded-lg p-4">
                              <div className="flex items-center justify-between">
                                  ...
                                  <div>
                                    <div className="flex items-center space-x-2">
                                      <h3 className="font-medium text-white">
                                        {session.deviceInfo?.browser || 'Browser'} Session
                                      </h3>
                                      {index === 0 && (
                                ...
                                {index !== 0 && (
                                  <button
                                    onClick={() => setShowDeleteSessionConfirm(session.sessionToken)}
                                    className="text-red-400 hover:text-red-300 p-2"
                                  >
                                    <X className="w-4 h-4" />
                                  </button>
