...
echo "   → Authorization callback URL: https://equators.tech/api/auth/callback/github"
echo ""
echo "   Google OAuth 2.0 Client Settings:"
echo "   → Authorized redirect URIs: https://equators.tech/api/auth/callback/google"
echo ""

# Common AccessDenied causes
echo "🚨 Common AccessDenied Causes:"
echo "   1. CALLBACK URL MISMATCH"
echo "      - GitHub: Must be exactly https://equators.tech/api/auth/callback/github"
echo "      - Google: Must be exactly https://equators.tech/api/auth/callback/google"
echo ""
...
echo "      - Ensure equators.tech is verified/authorized in OAuth app settings"
echo ""
...
echo "   → Verify Authorization callback URL: https://equators.tech/api/auth/callback/github"
echo ""
echo "   Google:"
echo "   → Google Cloud Console → APIs & Services → Credentials → [Your OAuth Client]"
echo "   → Verify Authorized redirect URIs includes: https://equators.tech/api/auth/callback/google"
echo ""

echo "✅ If all settings are correct and issue persists:"
echo "   - Check server logs: pm2 logs equators-production"
